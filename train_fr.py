...
class Translation(pl.LightningModule):
    ...
    def beam_search(self, batch, max_length, num_beams, num_return_sequences):
        ...
        batch['encoder_outputs']["last_hidden_state"] = \
            batch['encoder_outputs'].last_hidden_state.index_select(
                0, expanded_return_idx)
        ...
        while True:
            ...
            next_token_logits[:, self.model.config.pad_token_id] = float(
                "-inf")
...
if __name__ == "__main__":
    ...
    transformer_model = transformers.MarianModel.from_pretrained(
        model_checkpoint)
    ...
    s3 = S3FileSystem(key='AKIA4QB2WTN57SCTNAGG',
                      secret='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')
    print("loading train data")
    train_data = datasets.load_from_disk(
        's3://mtacl/tokenized_data_fr/train_ver', fs=s3)
    print("loading test data")
    test_data = datasets.load_from_disk(
        's3://mtacl/tokenized_data_fr/test_ver', fs=s3)
    print("loading validation data")
    val_data = datasets.load_from_disk(
        's3://mtacl/tokenized_data_fr/validation_ver', fs=s3)
